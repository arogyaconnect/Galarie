  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        const artTokenNetworkData = ArtToken.networks[networkId];
        const artMarketplaceNetworkData = ArtMarketplace.networks[networkId];

        if (artTokenNetworkData && artMarketplaceNetworkData) {
          const artTokenContract = new web3.eth.Contract(
            ArtToken.abi,
            artTokenNetworkData.address
          );
          const marketplaceContract = new web3.eth.Contract(
            ArtMarketplace.abi,
            artMarketplaceNetworkData.address
          );
          ...
          alert(`Contracts not deployed to the current network: ${networkId}`);
        }
      } catch (error) {
        alert(
